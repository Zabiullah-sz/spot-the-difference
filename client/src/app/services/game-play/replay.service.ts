...
export class ReplayService {
    ...
    store(action: () => void) {
        ...
        console.log(this.gameTimeService.replayTime);
    }
    ...
    // Function to jump to a specific percentage of the replay
    goToPercentage(percentage: number): void {
        this.restart();
        const startTime = this.gameTimeService.recordedTimes[0];
        const endTime = this.gameTimeService.recordedTimes[this.gameTimeService.recordedTimes.length - 1];

        const targetTime = startTime - ((startTime - endTime) * percentage) / 100;
        const closestActionIndex = this.findClosestAction(targetTime);
        if (closestActionIndex !== -1) {
            this.actionIndex = closestActionIndex;
            this.gameTimeService.setReplayTime(targetTime);
        }
    }

    // Helper function to find the closest action to a given time
    private findClosestAction(time: number): number {
        for (let i = 0; i < this.replayActions.length; i++) {
            if (this.replayActions[i].time <= time) {
                return i;
            }
        }
        return -1;
