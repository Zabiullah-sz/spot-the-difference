...
export default class ClassicSingleplayer extends Game {
    ...
    async endGame(winner?: Player) {
        ...
        if (this.isOngoing) {
            ...
            if (winner) {
                ...
                const clientId = winner.client.id;
                const userId = this.userSessionService.getUser(clientId).userId;
                await this.userService.addPoints(userId, winner.differencesFound);
    ...
    verifyClick(playerId: string, clickCoordinates: Coordinates): boolean {
        return super.verifyClick(playerId, clickCoordinates, (foundDifferenceValues: GameDifferenceImages, player: Player) => {
            if (foundDifferenceValues) {
                ...
                this.sendToObservers(OutputFilterGateway.sendClickResponseMessage, validClickResponse);
    ...
    sendToObservers(filter: EmissionFilter<unknown>, message: unknown) {
        for (const observer of this.playerGroup.getObservers) {
            filter.toClient(observer.client, message);
        }
    }
}
