...
export default class GameAuthorityService {
    ...
    static get getOngoingGamesArray(): ActiveGame[] {
        return this.ongoingGames.getGames;
    }
    ...
    static async connect(connectionData: GameConnectionData) {
        if (connectionData.startEarly !== true) if (this.isPlaying(connectionData.user.client.id)) return;
        if (this.joinPendingGame(connectionData)) return;
        switch (connectionData.gameMode) {
            case GameMode.Classic1v1: {
                const successfullyAdded = await new Classic1v1(this.mongoDBService, this.userService, this.userSessionService).initialize(
                    connectionData,
                );
            ...
            case GameMode.ClassicSolo: {
                await new ClassicSingleplayer(this.mongoDBService, this.userService, this.userSessionService).initialize(connectionData);
                break;
            }
            case GameMode.LimitedTimeCoop: {
                await new LimitedTimeCoop(this.mongoDBService, this.userService, this.userSessionService).initialize(connectionData);
                break;
            }
            case GameMode.LimitedTimeSolo: {
                await new LimitedTimeSolo(this.mongoDBService, this.userService, this.userSessionService).initialize(connectionData);
                break;
            }
        }
    }

    static async connectToChat(connectionData: GameConnectionData) {
        const successfullyAdded = await new Classic1v1(this.mongoDBService, this.userService, this.userSessionService).initialize(connectionData);
        if (successfullyAdded) {
            this.joinableGames.push(connectionData.cardId);
            OutputFilterGateway.sendJoinableGames.toServer(this.joinableGames);
        }
    ...
    static joinPendingGame(data: GameConnectionData): boolean {
        ...
        switch (data.gameMode) {
            ...
            case GameMode.LimitedTimeCoop:
                func = (game: Game): boolean => {
                    if (game.getGameMode === data.gameMode) {
                        if (data.startEarly) {
                            game.startGame();
                            return true;
                        } else if (game.join(data.user)) return true;
                    }
                    return false;
                };
                break;
