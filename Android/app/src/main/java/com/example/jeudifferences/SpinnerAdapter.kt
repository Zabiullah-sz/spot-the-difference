    ...
    : ArrayAdapter<ColorObject>(context, 0 , list)
{
    ...
    override fun getDropDownView(position: Int, convertView: View?, parent: ViewGroup): View {
        ...
        return createView(position, cv!!, parent)
    }
    ...
    private fun createView(position: Int, view: View, parent: ViewGroup): View {
        ...
        val colorImage = view.findViewById<ImageView>(R.id.imageView)

        colorNameBG?.text = colorObject.name
        colorNameItem?.text = colorObject.name
        colorImage?.setImageResource(colorObject.resourceId)
...
class SpinnerAdapter (context: Context, list : List<ColorObject>): ArrayAdapter<ColorObject>(context, 0 , list){
}
class ColorSpinnerAdapter(context: Context, list : List<ColorObject>)
    : ArrayAdapter<ColorObject>(context, 0 , list)
{
